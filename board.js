...
var Board = function(size) {
  ...
  //---------------------------------------------------------MY CODE FOR YELLOW ARROW PROCESS-----------------------------------------------------------
	/**
	 * Draw an arrow from previous checker location to new location on the canvas
	 */
   function drawArrow(rowFrom, colFrom, rowTo, colTo){
        //need to convert origin and destination row,col nums into pixel coordinates
        var unit = 400 / sizeN; //size of 1 unit in pixels:
        console.log(unit);
        initialY = (rowFrom + 0.5) * unit;
        initialX = (colFrom + 0.5) * unit;
        finalY = (rowTo + 0.5) * unit;
        finalX = (colTo + 0.5) * unit;
        ...
        //Clearing previous arrow before drowing new arrow
        ctxt.clearRect(0, 0, canvas.width, canvas.height);
        ctxt.save();

        //Drawing new arrow (This part up to the closure was adapted from stack overflow)
        ctxt.beginPath();
        var headlen = 10;   // length of head in pixels
        var angle = Math.atan2(finalY-initialY,finalX-initialX);
        ctxt.moveTo(initialX, initialY);
        ctxt.lineTo(finalX, finalY);
        ctxt.lineTo(finalX-headlen*Math.cos(angle-Math.PI/6),finalY-headlen*Math.sin(angle-Math.PI/6));
        ctxt.moveTo(finalX, finalY);
        ctxt.lineTo(finalX-headlen*Math.cos(angle+Math.PI/6),finalY-headlen*Math.sin(angle+Math.PI/6));
        
        //Closing canvas editing
        ctxt.closePath();
        ctxt.fill();
        ctxt.stroke();
        ctxt.restore();
	...
	this.moveTo = function(checker, toRow, toCol){
      ...
      //moveTo is called every time any checker is moved on board so I draw the arrow here to make sure it is always drawn
      drawArrow(checker.row, checker.col, toRow, toCol);
